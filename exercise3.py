...
with open("results/phonebook_raw.csv", encoding="utf-8") as f:
    rows = csv.reader(f, delimiter=",")
    contacts_list = list(rows)


# TODO 1: выполните пункты 1-3 ДЗ
#функция приведения телефонов к одному формату

@logger("results/log_phonebook.log")
def create_contacts(contact_list: list):
    ...
    return union_contacts(new_contact_list)


# функция обработки повторов и пустых записей
@logger("results/log_phonebook.log")
def union_contacts(contacts: list):
...
with open("results/phonebook.csv", "w", encoding="utf-8") as f:
    datawriter = csv.writer(f, delimiter=',')
    datawriter.writerows(create_contacts(contacts_list))
